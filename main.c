...

{
	...
	send_a_string("ALARMCLOCK BY N6");
	_delay_ms(250);
	send_a_command(0x01);
	...
	{	
		...
		{ 
				...
				while(1)
				{
					...
					if (bit_is_set(PINA, 1))  // Nút t?ng
					{
						if (dayss < 7) dayss++;  // T?ng gi
						else  dayss = 0;  // ??t l?i v? 0 n?u gi? ??t 24
										  // Th?i gian tr? ?? tránh nh?n nhi?u l?n
					}
					if (bit_is_set(PINA,2))  // Nút gi?m
					{
						if (dayss > 0) dayss--;  // Gi?m gi?

						else dayss = 6;  // ??t l?i v? 23 n?u gi? ??t 0
